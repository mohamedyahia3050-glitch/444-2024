...
function makeBanner(hasNotes){
  if(hasNotes) return '<div class="py-2 px-4 rounded-t-lg text-white font-semibold" style="background:linear-gradient(90deg,#16a34a,#059669)"><span>قرارات التعديل الصادرة</span></div>';
  ...
  return `
    <div class="mb-3 rounded-lg overflow-hidden shadow" id="card-${idx}">
      ...
      <div class="p-4 bg-white text-right">
        ...
        <p class="mb-1"><strong>القرارات الصادرة:</strong> ${ item.attachments ? '<div class="whitespace-pre-line mt-2 text-sm">'+escapeHtml(item.attachments)+'</div>' : '<span class="text-red-600">لا توجد قرارات</span>' }</p>
      </div>
    </div>
  `;
}
